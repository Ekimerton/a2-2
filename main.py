...
def forwardFT( image ):
  new_image = np.zeros(image.shape, dtype=np.complex128)
  for i in range(len(image)):
    new_image[i] = ft1D(image[i])
  
  for i in range(len(image[0])):
    new_image[:,i] = ft1D(new_image[:,i])
...
def inverseFT( image ):
  M, N = image.shape
  image_conj = np.conj(image)
  new_image = np.zeros(image.shape, dtype=np.complex128)

  for u in range(M):
    new_image[u] = ft1D(image_conj[u])

  for v in range(N):
    new_image[:, v] = ft1D(new_image[:, v])

  new_image = np.conj(new_image) / (M * N)
  return new_image
...
def multiplyFTs( image, filter ):
  ...
  M, N = image.shape
  filter_shift = np.zeros(filter.shape, dtype=np.complex128)

  for u in range(M):
    for v in range(N):
        filter_shift[u][v] = filter[u][v] * np.exp(2 * np.pi * 1j * (u * (M/2)/M))
        filter_shift[u][v] = filter_shift[u][v] * np.exp(2 * np.pi * 1j * (v * (N/2)/N))

  return image * filter_shift
...
def keyboard( key, x, y ):
  ...
  # README: A little bug here that I found for y'all. ['a', 's'] doesn't work
  ...
  if editMode == 'a':
    for i in range(-radius, radius):
      for j in range(-radius, radius):
        ...
        wrapped_x = int(wrap(x+j, image.shape[1]))
        wrapped_y = int(wrap(y+i, image.shape[0]))
        gauss = np.exp(-1 * ((i**2 + j**2) / (2 * radius**2)))

        if isFT:
          image[wrapped_y][wrapped_x] = np.exp(np.log(image[wrapped_y][wrapped_x]) * (1 + 0.1 * gauss))
        else:
          image[wrapped_y][wrapped_x] = image[wrapped_y][wrapped_x] * (1 + 0.1 * gauss)
    ...
    for i in range(-radius, radius):
      for j in range(-radius, radius):
        ...
        wrapped_y = int(wrap(y+i, image.shape[0]))
        gauss = np.exp(-1 * ((i**2 + j**2) / (2 * radius**2)))
        print(wrapped_x, wrapped_y)
       
        if isFT:
          image[wrapped_y][wrapped_x] = np.exp(np.log(image[wrapped_y][wrapped_x]) * (1 - gauss))
        else:
          image[wrapped_y][wrapped_x] = image[wrapped_y][wrapped_x] * (1 - gauss)
